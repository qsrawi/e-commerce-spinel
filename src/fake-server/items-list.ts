...
import { ItemsList } from '../app/shared/interfaces/list';
import {
    CategoryFilterItem,
 ...
 */
export function getItemsList(itm:Item[], options: ListOptions): Observable<ItemsList>{
    ...
    items = items.sort((a, b) => {
        if (['name_asc', 'name_desc'].includes(sort)) {
            if ( a.ItemEnName === b.ItemEnName ) {
                return 0;
            }

            return (a.ItemEnName > b.ItemEnName ? 1 : -1) * (sort === 'name_asc' ? 1 : -1);
        }
...
// export function getItemsList(categorySlug: string|null, options: ListOptions): Observable<ItemsList> {
//     const page = options.page || 1;
//     const limit = options.limit || 12;
//     const sort = options.sort || 'default';
//     const filterValues = options.filterValues || {};
 ...
 */
function getFilterValue(type: 'range', slug: string, product: Item, defaultValue: number|null): number;
function getFilterValue(type: 'check', slug: string, product: Item, defaultValue: FilterListValueDef[]): FilterListValueDef[];
function getFilterValue(type: string, slug: string, product: Item, defaultValue: FilterValueDef|null = null): FilterValueDef|null {
...
function getRangeValue(slug: string, product: Item, defaultValue: number|null = null): number {
    return getFilterValue('range', slug, product, defaultValue);
}

function getListValues(slug: string, product: Item, defaultValue: FilterListValueDef[] = []): FilterListValueDef[] {
...
function testProduct(filter: Filter, product: Item): boolean {
...
function calcProductsForFilterValues(filter: Filter, allFilters: Filter[], products: Item[]): ProductsForFilterValuesResult {
...
function makeFilters(filtersDef: FilterDef[], products: Item[]): Filter[] {
    const result: Filter[] = [];
