...
export class CartCampaignsandOffersService {
  ...
  removeall(): Observable<void> {
    // timer only for demo
    localStorage.removeItem('cartItemsNewCampaigns');
    this.data.items = [];
  ...
  private calc(): void {
    ...
    totals.push({
        title: 'Discount '+(this.data.items[0]?.product.campaign_DiscountPercentage!=undefined?this.data.items[0]?.product.campaign_DiscountPercentage:0) +'%',
        price: subtotal * ((this.data.items[0]?.product.campaign_DiscountPercentage!=undefined?this.data.items[0]?.product.campaign_DiscountPercentage:0)/100),
        type: 'disc'
    });

    const total = subtotal + totals.reduce((acc, eachTotal) => acc + (eachTotal.type=='disc'?(-eachTotal.price):eachTotal.price), 0);
  ...
  private save(): void {
    localStorage.setItem('cartItemsNewCampaigns', JSON.stringify(this.data.items));
  }
  ...
  private load(): void {
    ...
    //     // this.itemsSubject$.next(this.data.items);
    // }
    const items = localStorage.getItem('cartItemsNewCampaigns');
